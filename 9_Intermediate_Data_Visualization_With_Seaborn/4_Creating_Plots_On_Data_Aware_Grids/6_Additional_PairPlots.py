...
# This exercise will go through a couple of more examples of how the pairplot() can be customized for quickly analyzing
# data and determining areas of interest that might be worthy of additional analysis.

# One area of customization that is useful is to explicitly define the x_vars and y_vars that you wish to examine.
# Instead of examining all pairwise relationships, this capability allows you to look only at the
# specific interactions that may be of interest.

# We have already looked at using kind to control the types of plots. We can also use diag_kind to control
# the types of plots shown on the diagonals.
# In the final example, we will include a regression and kde plot in the pairplot.


# Create a pair plot that examines fatal_collisions_speeding and fatal_collisions_alc on the x-axis
...
sns.pairplot(data=df, vars=["insurance_losses", "premiums"], kind='reg',
             palette='BrBG', diag_kind = 'kde', hue='Region')
# Hint: Variables are defined by vars will be automatically split across the x and y axis.
